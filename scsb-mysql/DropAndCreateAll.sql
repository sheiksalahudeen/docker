...
CREATE TABLE IF NOT EXISTS `NOTES_T` (
  ...
  CONSTRAINT `NOTES_ITEM_ID_FK` FOREIGN KEY (`ITEM_ID`) REFERENCES `item_t` (`ITEM_ID`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `NOTES_REQUEST_ID_FK` FOREIGN KEY (`REQUEST_ID`) REFERENCES `request_item_t` (`REQUEST_ID`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB;
...
CREATE TABLE IF NOT EXISTS `REQUEST_ITEM_T` (
  ...
  CONSTRAINT `REQUEST_PATRON_ID_FK` FOREIGN KEY (`PATRON_ID`) REFERENCES `PATRON_T` (`PATRON_ID`) ON DELETE NO ACTION ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS `MATCHING_BIB_INFO_DETAIL_T` (
  ...
  KEY `RECORD_NUM_IDX` (`RECORD_NUM`)
) ENGINE=InnoDB;

DROP TABLE IF EXISTS REQUEST_INST_BIB_T;

CREATE TABLE IF NOT EXISTS REQUEST_INST_BIB_T (
  ITEM_ID INT(11) NOT NULL,
  OWNING_INST_BIB_ID VARCHAR(45) COLLATE NOT NULL,
